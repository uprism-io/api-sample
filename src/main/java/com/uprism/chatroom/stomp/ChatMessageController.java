package com.uprism.chatroom.stomp;
...
import com.uprism.chatroom.model.ChatMessage;
...
public class ChatMessageController {
    ...
    public void conference(ChatMessage message, @DestinationVariable String confTitle) throws JSONException {
    	JSONObject conf = new SendApi().makeConf(message.getMessage(), confTitle, "", apiAddress);
    	message.setMessage((conf.getJSONObject("response").toString()));
    	template.convertAndSend("/subscribe/conf/room/" + message.getChatRoomId(), message);
    }
    
    @MessageMapping("/conference/{confTitle}/{confAgenda}")
    public void conference(ChatMessage message, @DestinationVariable String confTitle, @DestinationVariable String confAgenda) throws JSONException {
    ...
    public void joinConference(ChatMessage message) throws JSONException {
    	JSONObject conf = new SendApi().joinConf(message.getWriter(), message.getMessage(), apiAddress);
    	message.setMessage(conf.getString("response"));
